...
const Walkthrough2 = ({animate}) => {
  ...
  const motiImage6 = useDynamicAnimation(() => ({top: '52%', left: '60%'}));

  useEffect(() => {
    if (animate) {
      motiImage1.animateTo({top: '20%', left: '15%'});
      motiImage2.animateTo({top: '38%', left: -10});
      motiImage3.animateTo({top: '65%', left: '5%'});
      motiImage4.animateTo({top: '80%', left: '55%'});
      motiImage5.animateTo({top: '20%', left: '65%'});
      motiImage6.animateTo({top: '58%', left: '80%'});
    }
  }, [animate]);
